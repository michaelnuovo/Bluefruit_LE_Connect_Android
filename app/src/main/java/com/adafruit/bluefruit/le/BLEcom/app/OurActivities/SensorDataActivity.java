...
public class SensorDataActivity extends UartInterfaceActivity implements SensorEventListener, GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, LocationListener {

    // Config
    private final static boolean kKeepUpdatingParentValuesInChildActivities = true;

    // Log
    private final static String TAG = SensorDataActivity.class.getSimpleName();
...
//    };
    ...
    private Runnable mPeriodicallySendData = new Runnable() {
        ...
        public void run() {
            ...
            for (int sensor_idx = 0; sensor_idx < mSensorData.length; sensor_idx++) {
                SensorData sensorData = mSensorData[sensor_idx];
                ...
                if (sensorData.enabled && sensorData.values != null) {
                    ...
                    ByteBuffer buffer;
                    if(sensor_idx == 1) buffer = ByteBuffer.allocate(4 + sensorData.values.length * 4).order(java.nio.ByteOrder.LITTLE_ENDIAN);
                    ...
                    // Add the ID
                    if(sensor_idx == 0) buffer.put((byte) PacketUtils.PacketTypes.QUAT.ordinal());
                    if(sensor_idx == 1) buffer.put((byte) PacketUtils.PacketTypes.ACCEL.ordinal());
                    if(sensor_idx == 2) buffer.put((byte) PacketUtils.PacketTypes.GYRO.ordinal());
                    if(sensor_idx == 3) buffer.put((byte) PacketUtils.PacketTypes.MAG.ordinal());
                    if(sensor_idx == 4) buffer.put((byte) PacketUtils.PacketTypes.MAG.ordinal());

                    // values
                    for (int values_idx = 0; values_idx < sensorData.values.length; values_idx++)
                        buffer.putFloat(sensorData.values[values_idx]);

                    if(sensor_idx == 1){ // Only applies packets counter to accelerometer
                        buffer.put((byte) packetCounter);
                        Log.v("TAG","Adding packet counter, it's value is "+String.valueOf(packetCounter));
                        packetCounter++;
                    }

                    Log.v(TAG,"packet counter value is "+String.valueOf(packetCounter));


                    byte[] result = buffer.array();
                    Log.d(TAG, "Send data for sensor: " + sensor_idx);
